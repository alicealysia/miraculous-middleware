...
export default async (request: Request<any, any, any, {referralId?: number, clientId?: number}>, response: Response, next: NextFunction) => {
    try {
        if (request.query.referralId) {
            const filter = await new accessControl(request.User).read(AccessControl.Resource.referral).id(request.query.referralId);
            const referral = await new typeorm(Entity.Referral).findOne(request.query.referralId);
            return response.json(filter(referral));
        }
        if (!request.query.clientId) {
            throw new Error('no referral or client specified');
        }
        const referrals = await new accessControl(request.User).read(AccessControl.Resource.referral).list();
